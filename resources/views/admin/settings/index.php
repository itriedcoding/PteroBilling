...
<div class="max-w-5xl mx-auto">
    ...
    <form method="POST" action="/admin/settings" class="space-y-6" x-data="{ activeTab: 'general' }">
        ...
        <div x-show="activeTab === 'general'" class="bg-white rounded-xl shadow-sm border p-8">
            <h3 class="text-xl font-bold mb-6">General & Domain Settings</h3>
            <div class="space-y-6">
                <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    <div class="flex items-start">
                        ...
                        <div class="text-sm text-blue-800">
                            ...
                            <p>To use a custom domain like <strong>billing.example.com</strong>:</p>
                            <ol class="list-decimal list-inside mt-2 space-y-1">
                                ...
                                <li>Update your PteroBilling .env APP_URL and APP_DOMAIN</li>
                            </ol>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Panel URL (Full URL)</label>
                    <input type="url" name="site_url" value="<?= htmlspecialchars($settings['site_url'] ?? $_ENV['APP_URL'] ?? '') ?>"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                        placeholder="https://billing.example.com">
                    <p class="mt-1 text-sm text-gray-500">Must include https://</p>
                </div>
                <div>
                    ...
                    <div class="flex">
                        ...
                        <input type="text" name="custom_domain" value="<?= htmlspecialchars($settings['custom_domain'] ?? $_ENV['APP_DOMAIN'] ?? '') ?>"
                            class="flex-1 px-4 py-3 border border-gray-300 rounded-r-lg focus:ring-2 focus:ring-blue-500"
                            placeholder="billing.example.com">
                    </div>
                    <p class="mt-1 text-sm text-gray-500">Your custom domain without https://</p>
                </div>
                <div>
                ...
                <div>
                    ...
                    <div class="bg-gray-50 rounded-lg p-4 space-y-4">
                        ...
                        <div class="bg-blue-50 border border-blue-200 rounded p-3 text-sm text-blue-800">
                            ...
                            Webhook URL: <code class="font-mono bg-blue-100 px-1 rounded"><?= htmlspecialchars($_ENV['APP_URL'] ?? 'https://billing.example.com') ?>/api/v1/payment/stripe</code><br>
                    ...
                    <div class="bg-gray-50 rounded-lg p-4 space-y-4">
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="password" name="paypal_client_secret" value="<?= htmlspecialchars($settings['paypal_client_secret'] ?? '') ?>"
                                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                            </div>
                        </div>
                        <div>
                        ...
                        <div class="bg-blue-50 border border-blue-200 rounded p-3 text-sm text-blue-800">
                            <i class="fas fa-info-circle mr-1"></i>
                            Webhook URL: <code class="font-mono bg-blue-100 px-1 rounded"><?= htmlspecialchars($_ENV['APP_URL'] ?? 'https://billing.example.com') ?>/api/v1/payment/paypal</code><br>
                ...
                <div>
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center">
                            ...
                            <h4 class="text-lg font-semibold">Credit System</h4>
                        </div>
                        <label class="flex items-center cursor-pointer">
                    ...
                    <div class="bg-gray-50 rounded-lg p-4">
                        <p class="text-sm text-gray-600">The credit system allows users to add funds to their account and use them to purchase/renew servers. This is always enabled as it's the core billing mechanism.</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div x-show="activeTab === 'pterodactyl'" x-cloak class="bg-white rounded-xl shadow-sm border p-8">
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <input type="url" name="ptero_url" value="<?= htmlspecialchars($settings['ptero_url'] ?? $_ENV['PTERODACTYL_URL'] ?? '') ?>"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                        placeholder="https://panel.example.com">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Application API Key</label>
                    <input type="password" name="ptero_api_key" value="<?= htmlspecialchars($settings['ptero_api_key'] ?? $_ENV['PTERODACTYL_API_KEY'] ?? '') ?>"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                    ...
                    <div class="flex items-start">
                        ...
                        <div class="text-sm text-blue-800">
                            ...
                            <ol class="list-decimal list-inside space-y-1">
                                ...
                                <li>Click <strong>Application</strong> in the top navigation</li>
                                <li>Click <strong>Create New</strong></li>
                                <li>Give it a name and select all permissions</li>
                                <li>Copy the key (it starts with <code>ptla_</code>)</li>
                            </ol>
                        </div>
                    </div>
                </div>
            ...
            <div class="space-y-6">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="mail_host" value="<?= htmlspecialchars($settings['mail_host'] ?? '') ?>"
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" placeholder="smtp.gmail.com">
                    </div>
                    <div>
                ...
                <div>
                    ...
                    <input type="text" name="mail_username" value="<?= htmlspecialchars($settings['mail_username'] ?? '') ?>"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" placeholder="noreply@example.com">
                </div>
                <div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="mail_from_name" value="<?= htmlspecialchars($settings['mail_from_name'] ?? 'PteroBilling') ?>"
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                    </div>
                </div>
            </div>
